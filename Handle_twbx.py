...
def xml_open(filename, expected_root=None):
    ...
    if zipfile.is_zipfile(filename):
        ...
        try:
            ...
            # todo: remove when Data Models are fixed
    ...
    with zipfile.ZipFile(filename) as zf:
        with zf.open(find_file_in_zip(zf)) as xml_file:
            try:
                ...
                # todo: remove when Data Models are fixed
                with temporary_directory() as temppath:
                    fixedfile = fix_namespace(xml_tree, temppath)
                    xml_tree = ET.parse(fixedfile)
...
def save_into_archive(xml_tree, filename, new_filename=None):
    ...
    with temporary_directory() as temp_path:
        ...
        xml_tree.write(os.path.join(
            temp_path, xml_file), encoding="utf-8", pretty_print=True, xml_declaration=True)
...
def save_file(container_file, xml_tree, new_filename=None):
    ...
    if zipfile.is_zipfile(container_file):
        ...
        xml_tree.write(new_filename, encoding="utf-8", pretty_print=True, xml_declaration=True)


def fix_namespace(filename, outpath):
    replacements = {' user:': ' '}

    with open(filename) as infile, open(os.path.join(outpath, Path(filename).name), 'w') as outfile:
        for line in infile:
            for src, target in replacements.items():
                line = line.replace(src, target)
            outfile.write(line)

    return os.path.join(outpath, Path(filename).name)
